package com.mobiledi.earnit.screenruletimepicker;
...
import static com.mobiledi.earnit.screenruletimepicker.TimeUtils.getStatetime;
...
public class TimeSelectUtils implements NumberPicker.OnValueChangeListener, View.OnClickListener {
    ...
    public TimeSelectUtils(Context activity, String initDateTime, TextView selectTime, TextView totalTime, GetSubmitTime subTime) {
    ...
    public Dialog dateTimePicKDialog() {
        View dateTimeLayout = View.inflate(activity, R.layout.screenrule_select,
                null);
    ...
    private void selectTimes() {
        if (rgIn.isChecked()) {
            ...
            if (!TimeUtils.compareTwoTime(startTime, endTime)) {
                Toast.makeText(activity, "Please select the correct playing time", Toast.LENGTH_SHORT).show();
            } else {
                endTime = initDateTime;
